        ...
        <div class="col-md-12">
            <div class="portlet light bordered" id="form_wizard_1">
                ...
                <div class="portlet-body">
                    ...
                    <form id="search_form"  name="search_form" action="${basePath }qier/orderReportContent/loadMerchartOrder.do">
                        <table >
                            <tr>
                                <td>  <strong>精确查询 :</strong>  </td>
                                <td>  &nbsp;&nbsp;开始:  &nbsp;&nbsp;</td>
                                <td width="155px">
                                    <input style="width: 180px;" readonly="readonly" type="text" id="selStartDate" name="selStartDate" class="form-control date_m" datefmt="yyyy-MM-dd HH:mm:ss" >
                                </td> 
                                <td>&nbsp;--到--&nbsp;&nbsp;</td>
                                <td>
                                    <input style="width: 180px;" readonly="readonly" type="text" id="selStopDate" name="selStopDate"  class="form-control date_m" datefmt="yyyy-MM-dd HH:mm:ss">
                                </td>
                            </tr>
                    ...
                    <table border="0">
                        ...
                        <tr>
                            <td></td>
                            <td><font color="red">  平台红包</font></td>
                            <td> : <span id="terrace_time_redPay"></span></td>
                            <td>元 /</td>
                            <td> <span id="terrace_time_redPay_size"></span></td>
                            <td>笔</td>
                            <td>(付款成功)</td>
                        </tr>
                        <tr>
                            <td></td>
                            <td><font color="red">  本店红包</font></td>
                            <td> : <span id="merchant_time_redPay"></span></td>
                            <td>元 /</td>
                            <td> <span id="merchant_time_redPay_size"></span></td>
                            <td>笔</td>
                            <td>(付款成功)</td>
                        </tr>
                        <tr>
...
<script type="text/javascript">
    function downloadExcelByTime() {
        ...
        $.post("${basePath}/qier/orderReportContent/excelFileDownload.do",request_data,function (data) {
    ...
    function selDataQuick(scope){
        ...
        $.post("${basePath}/qier/orderReportContent/selDataQuick.do",request_data,function (data) {
            layer.open({
                type: 1,
    ...
    function getCollectDataByTime(){
        ...
        // 请求收益数据
        $.post("${basePath}/qier/orderReportContent/getCollectDataByTime.do",request_data,function (data) {
            data  = JSON.parse(data);
            ...
            if (data.statusCode == 300){
                ...
                $("#merchant_time_redPay").html(data.data.merchant_price_fen_yorz/100);
                $("#merchant_time_redPay_size").html(data.data.merchant_price_fen_yorz_size);

                $("#terrace_time_redPay").html(data.data.terrace_price_fen_yorz/100);
                $("#terrace_time_redPay_size").html(data.data.terrace_price_fen_yorz_size);
    ...
    function refundById(tradeRecord_id){
    	...
    	ajax_open(basePath+"merchant/business_page/BusinessRecord/TradeRecord72_refund.jsp?tradeRecord_id="+tradeRecord_id,function(){
    ...
    function print_tradeRecord_id(tradeRecord_id){
        ...
        ajax_post(
            ...
            function (message) {
                ...
                if(data_return.length > 0){
                    var tradeRecord_tmp = data_return[0];
                    
                    if(tradeRecord_tmp.state == STATE_OK ||  tradeRecord_tmp.state == STATE_PAY_SUCCESS){
                        printed_tradeRecord_id(tradeRecord_id);
                        print_tradeRecord(tradeRecord_tmp);
                    }else{
                    	//询问框
                    	var confirm_print = layer.confirm('当前订单还未付款成功!', {
                    	  btn: ['确认打印','取消打印'] //按钮
                    	}, function(){
                    		 printed_tradeRecord_id(tradeRecord_id);
                             print_tradeRecord(tradeRecord_tmp);
                             layer.close(confirm_print);
                    	}, function(){
                    		layer.close(confirm_print);
                    	});
                    }
    ...
    function editMemo(memo,id) {
        layer.open({
            type: 1,
            area: ['300px', '180px'],
            shadeClose: true, //点击遮罩关闭
            content: '\<\div style="padding:20px;"><input type="text" id="newMemo" value="'+memo+'" class="form-control">\<\/div>',
            btn:['完成','关闭'],
            btn1:function () {
                var newMemo = $("#newMemo").val();
                $.post("${basePath}/qier/orderReportContent/updateMemo.do",{"newMemo":newMemo,"id":id},function (data) {
                    data = JSON.parse(data)
                    if (data.statusCode == 200){
                        var url = '${basePath}merchant/business_page/BusinessRecord/TradeRecord72_list_content.jsp';
                        linkToPage(url);
                        layer.closeAll();
                    }
                });
