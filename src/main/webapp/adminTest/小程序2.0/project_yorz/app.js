  ...
  _login: function (nextFun) {
    ...
    const success_server = (res) => {
      ...
      if (res.userId){
        ...
        this.globalData.userInfo.city = res.cityName

        console.log(this.globalData.userInfo.city);
        console.log("调试输出:----"+res.print_message);

        // this.globalData.shopTypes = res.labelList || [] // 店铺分类
        if (res.label_list) { // 店铺分类
          this.globalData.shopTypes.splice(1, this.globalData.shopTypes.length) // 清空

          res.label_list.forEach((item, index, array) => {
            this.globalData.shopTypes.push({
              id: index + 1,
              name: item,
              order: index + 1,
      ...
      wx.reLaunch({
          url: '/pages/index/home?city=' + this.globalData.userInfo.city + '&tabbarIndex=0'
        })
    }
    /**
     * 服务端调用登陆失败
