...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.hasOwnProperty('city')) {
      ...
      wx.request({
        url: "https://tiesh.liebianzhe.com/appletV2/getLabel.do",
        // url: "http://test.liebianzhe.com/appletV2/getLabel.do",
        // url: "http://192.168.1.101:8080/appletV2/getLabel.do",
        header: {
          "Content-Type": "application/x-www-form-urlencoded"
        },
        method: "POST",
        data: { cityName: options.city },
        complete: function (res) {
          if (res == null || res.data == null) {
            ...
            if (res.data.labelList) { // 店铺分类
              app.globalData.shopTypes.splice(1, app.globalData.shopTypes.length) // 清空

              res.data.labelList.forEach((item, index, array) => {
                app.globalData.shopTypes.push({
                  id: index + 1,
                  name: item,
                  order: index + 1,
                })
  ...
  handleTapShopMap: function (e) {
    ...
    if (latitude && latitude > 0) {
      app.Tools.API_wx.openLocation(latitude, longitude)
    }
  },
  /**
   * 载入搜索列表下一页
