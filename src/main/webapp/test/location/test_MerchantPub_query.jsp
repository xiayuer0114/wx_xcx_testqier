...
<%   
	...
	//添加查询条件
	MongoCommand.nearSphere(merchantPub, longitude, latitude);
	//默认就是采用距离排序	把id 排序这些去掉
	merchantPub.setIs_sort(false);
	
	PageSplit pageSplit = new PageSplit();
		
	List<MerchantPub> merchantRedEnvelope_ite  = serializContext.findAll(merchantPub,pageSplit);
	
	for(MerchantPub merchantRedEnvelope_next:merchantRedEnvelope_ite){
		...
		System.out.println(merchantRedEnvelope_next.getName()+" "+distance);
		out.println(merchantRedEnvelope_next.getName()+" "+distance+"<br/>");
	}
