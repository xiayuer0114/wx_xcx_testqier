    ...
    pageEncoding="UTF-8"%><%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
		<fieldset  >
			...
			<dl>
				...
				<dd>
						<select name="user.payPwdState" style="width: 155px;" class="required combox" svalue="${user.payPwdState }">
								<option value="<%=User.STATE_NOTOK %>"  >不启用支付密码</option>
								<option  value="<%=User.STATE_OK %>" >启用支付密码</option>
						</select> 
				</dd>			</dl>
			<c:if test="${empty user.id}">
			<div class="divider"></div> 
			<dl>
					<dt>支付密码：</dt>
					<dd>
						<input type="password" name="pay_password" value=""  autocomplete="off" > 
					</dd>
				</dl> 
				<dl>
					<dt>密码确认：</dt>
					<dd>
						<input type="password" name="pay_passwordRe" value=""   autocomplete="off"> 
					</dd>
				</dl> 
			</c:if>
			<div class="divider"></div> 
