...
Long current_balance = cmbBankAccountPay.queryAccountBalanceFromCmb();
...
<div class="pageContent" id="${currentTimeMillis }"  >
	<form method="post" action="${basePath }v2/TransferToMerchantRecord/transfer_to_merchant.do"class="pageForm required-validate" onsubmit="return validateCallback(this, navTabAjaxDone);">
		<input type="hidden" name="merchant72_id" value="${user.id }"   >
		<input type="hidden" name="id" value="${user.id }"   >
		<input type="hidden" name="requestFlow" value="<%=System.currentTimeMillis() %>"   >
		<input type="hidden" name="discount_ratio" value="${user.discount_ratio/1000000 }"   >
		 ...
		 <fieldset >
			...
			<div class="divider"></div>
		 	<dl>
				<dt>账户余额</dt>
				<dd> 
				${cmbBankAccountPay.AVLBLV_fen/100 }元
				</dd>
			</dl>  
			<div class="divider"></div>
			<dl>
			...
			<dl>
				<dt>转账金额：</dt>
				<dd> 
					 <input onblur="transter_price_yuan_change(this)" type="text" name="transter_price_yuan" value="${user.inOneDayLiuShui*5 }" class="digits required" >
				</dd>
			</dl>
			<div class="divider"></div>
			<dl>
